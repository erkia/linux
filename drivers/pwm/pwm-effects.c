...
static void pwm_effects_get_state(struct pwm_chip *chip, struct pwm_device *pwm, struct pwm_state *state)
{
    struct pwm_effects_drvdata *pdata = container_of(chip, struct pwm_effects_drvdata, chip);
...
static int pwm_effects_probe(struct platform_device *pdev)
{
    ...
    pdata->pwm = devm_of_pwm_get(&pdev->dev, np, NULL);
    if (IS_ERR (pdata->pwm)) {
        if (PTR_ERR(pdata->pwm) == -EPROBE_DEFER) {
            goto defer;
        }
        dev_err(&pdev->dev, "devm_of_pwm_get() failed: %ld\n", PTR_ERR(pdata->pwm));
        pdata->pwm = NULL;
    }
